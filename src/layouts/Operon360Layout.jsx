...
function O360Nav() {
  ...
  return (
    ...
    >
        ...
        <nav aria-label="Operon 360 primary" className="hidden items-center gap-7 lg:flex">
          {O360_NAV.map((item) => (
            <NavLink
              ...
              className={({ isActive }) =>
                `text-sm transition-colors duration-180 ${
                  isActive && !item.to.includes('#') ? 'text-paper' : 'text-paper-55 hover:text-paper'
                }`
              }
            >
              {item.label}
      ...
      {open && (
        <div className="border-t border-paper-14 bg-ink lg:hidden">
          <Container className="py-6">
            <nav aria-label="Operon 360 mobile" className="flex flex-col">
              {O360_NAV.map((item) => (
                <NavLink key={item.label} to={item.to} className="border-b border-paper-08 py-3.5 font-display text-h4 font-semibold">
                  {item.label}
                </NavLink>
              ))}
            </nav>
